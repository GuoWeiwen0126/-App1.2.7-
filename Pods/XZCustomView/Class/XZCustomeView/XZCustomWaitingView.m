                          ...
                          isNeedAdaptive:(BOOL)isNeedAdaptive
{
    ...
    UIFont *fontPromptText = [UIFont systemFontOfSize:14];
    // 设定绘制字符串高和宽
    ...
    float alertOvalViewWidth = 120;
    float alertOvalViewHeight = 120;
    ...
    // 设置圆角
    alertOvalView.layer.cornerRadius = 16;
    ...
    if (iconName) {
        ...
        imageView.frame = CGRectMake((alertOvalViewWidth - imageView_W)/2.0, (alertOvalViewWidth - imageView_W)/5.0, imageView_W, imageView_H);
        
        imageView.animationImages = mutableArr;
        // FIXME: 动画加载频路 数值越小 动画越快  可根据自己项目需求修改
        [imageView setAnimationDuration:(iconNumber/8.0)];
...
+ (void)showIconWaitingMaskViewWithIcon:(NSString *)iconName iconNumber:(int)iconNumber{
    ...
    [mainImageView setAnimationDuration:(iconNumber/4.0)];
...
//    DebugLog(@"TOOLS: showAutoHidePromptView: string = %@, image = %@, duration = %f", string, image, duration);
    ...
    {
        alertView = [[UIView alloc] initWithFrame:CGRectMake(([UIScreen screenWidth]-width)/2.0 - 20, ([UIScreen screenHeight]-height)/2.0 - 20, width+40, height+20)];
    ...
    //设定圆角
    alertView.layer.cornerRadius = image == nil ? 8:16;
