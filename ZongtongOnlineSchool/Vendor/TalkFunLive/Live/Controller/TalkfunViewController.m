...
//            NSInteger num = round((self.scrollView.contentOffset.x) / self.scrollView.frame.size.width);
//
//            if (num==0) {
//                if (self.enable) {
//                    self.onlineLabel.hidden = NO;
...
- (void)orientationPortrait
{
    ...
    @synchronized (self) {
        ...
        NSLog(@" isorientationLandscape:%d  ori:%ld",self.isOrientationLandscape,[APPLICATION statusBarOrientation]);
        if (self.isOrientationLandscape) {
            self.isOrientationLandscape = NO;
        ...
        NSLog(@" isorientationLandscape22222:%d  ori:%ld",self.isOrientationLandscape,[APPLICATION statusBarOrientation]);
        //        }
...
- (void)orientationLandscape
{
    @synchronized (self) {
        ...
        //        if (APPLICATION.statusBarOrientation == UIInterfaceOrientationPortrait) {
        NSLog(@" isorientationLandscape33333:%d  ori:%ld",self.isOrientationLandscape,[APPLICATION statusBarOrientation]);
        if (!self.isOrientationLandscape) {
        ...
        NSLog(@" isorientationLandscape44444:%d  ori:%ld",self.isOrientationLandscape,[APPLICATION statusBarOrientation]);
        //        }
        self.view.bounds = CGRectMake(0, 0, ScreenSize.width, ScreenSize.height);
...
- (void)liveModeChange:(id)obj
{
    ...
    if(mode==TalkfunLiveModeDesktop){
...
- (void)voteNew:(id)obj
{
    ...
    NSString * message   = [NSString stringWithFormat:@"管理员 %@ 在%@发起了一个",nickname,content];
...
- (TalkfunMoveLabel*)onlineLabel
{
    if (_onlineLabel==nil) {
        ...
        _onlineLabel.frame =CGRectMake(self.view.frame.size.width - 65, self.cameraView.frame.size.height +self.cameraView.frame.origin.y +(self.cameraView.frame.size.height*0.8)  , 65, 25);
        ...
        _onlineLabel.backgroundColor = [UIColor colorWithRed:68 / 255.0 green:169 / 255.0 blue:248 / 255.0 alpha:1];;
        _onlineLabel.textAlignment = NSTextAlignmentCenter;
        _onlineLabel.textColor = [UIColor colorWithRed:18 / 255.0 green:106 / 255.0 blue:255 / 255.0 alpha:1];;
    }
    return _onlineLabel;
}
...
- (void)keyBoardDidShow:(NSNotification *)notification
{
    ...
    if (APPLICATION.statusBarOrientation == UIInterfaceOrientationPortrait){
        ...
        self.onlineLabel.frame = CGRectMake(self.view.frame.size.width - 65, self.view.frame.size.height -keyboardFrameEndRect.size.height -50, 65, 25);
    }
    ...
    [UIView animateWithDuration:0.1 animations:^{
        ...
        CGFloat Y =  [UIScreen mainScreen].bounds.size.height -  keyboardFrameEndRect.size.height -self.ScoreView.evaluateFrame.size.height+40;
...
- (TalkfunSignViewController*)sign{
    if (_sign==nil) {
        ...
        _sign.view.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width,  [UIScreen mainScreen].bounds.size.height);
...
- (void)XiaoBan
{
    ...
    /**
    //开启讲台模式
    [self.podium on:TALKFUN_EVENT_PODIUM_START callback:^(id obj) {
        dispatch_async(dispatch_get_main_queue(), ^{
            //视频view 隐藏
            [weakSelf.cameraView removeFromSuperview];
    ...
    //老师关闭了讲台模式
    [self.podium on:TALKFUN_EVENT_PODIUM_STOP callback:^(id obj) {
        
        //上了讲台才处理
        if (weakSelf.podium.isUp) {
            dispatch_async(dispatch_get_main_queue(), ^{
                weakSelf.cameraView.hidden = NO;
                
                [weakSelf.podium down];
    ...
    //允许上讲台
    [self.podium on:TALKFUN_EVENT_PODIUM_UPWARDS callback:^(id obj) {
        
        dispatch_async(dispatch_get_main_queue(), ^{
            weakSelf.cameraView.hidden = YES;
            
            [weakSelf.podium upwards];
    ...
    //踢下讲台
    [self.podium on:TALKFUN_EVENT_PODIUM_KICK callback:^(id obj) {
        dispatch_async(dispatch_get_main_queue(), ^{
            
            
            [weakSelf.podium kick];
            
            //            for (UIView *vc in weakSelf.videoSessions) {
            //                if (vc.tag ==0) {
            //                    [vc removeFromSuperview];
            //
            //                    [weakSelf.videoSessions  removeObject:vc];
            //
            //                    return ;
            //                }
            //            }
    ...
    //主动下讲台
    [self.podium on:TALKFUN_EVENT_PODIUM_DOWN callback:^(id obj) {
        dispatch_async(dispatch_get_main_queue(), ^{
            //            weakSelf.cameraView.hidden = NO;
            [weakSelf.podium down];
    ...
    //打开摄像头
    [self.podium on:TALKFUN_EVENT_CAMERA_ON callback:^(id obj) {
        
        [weakSelf.podium camera:YES];
        
    }];
    //关闭摄像头
    [self.podium on:TALKFUN_EVENT_CAMERA_OFF callback:^(id obj) {
        
        [weakSelf.podium camera:NO];
        
    }];
    
    //打开麦克风
    [self.podium on:TALKFUN_EVENT_MICROPHONE_ON callback:^(id obj) {
        
        [weakSelf.podium muteLocalAudioStream:NO];
    }];
    //关闭麦克风
    [self.podium on:TALKFUN_EVENT_MICROPHONE_OFF callback:^(id obj) {
        
        [weakSelf.podium muteLocalAudioStream:YES];
    }];
    
    
    //打开画线权限
    [self.podium on:TALKFUN_EVENT_WHITEBOARD_ON callback:^(id obj) {
        
        [weakSelf.podium whiteboardPermissions:YES];
    }];
    
    //关闭画线权限
    [self.podium on:TALKFUN_EVENT_WHITEBOARD_OFF callback:^(id obj) {
        
        [weakSelf.podium whiteboardPermissions:NO];
    }];
...
- (void)podiumApply{
    //    WeakSelf
//    [self.podium  podiumApply:^(id result) {
//
//        if ([result[@"code"] intValue] == 0) {
//
//            //成功
//        }else  if ([result[@"code"] intValue] == 1220) {
//            //老师未开启讲台功能,举手无效
//            //你已申请上讲台，请不要重复申请
//
//        }
 ...
 * @ param从会话开始起经过的时间(毫秒)。
 */
- (void)remoteView:(UIView*)view firstLocalVideoFrameWithSize:(CGSize)size Uid:(NSInteger)uid
{
    
    for (UIView *vc  in self.videoSessions) {
        if (vc) {
    ...
    view.tag = uid;
    [self.videoSessions addObject:view];
    
    if (self.videoSessions.count ==1) {
        ...
        view.frame = CGRectMake(200, 300, 100, 100);
        [self.view addSubview:view];
    }
 ...
 * @ param从会话开始起经过的时间(毫秒)。
 */
- (void)remoteView:(UIView*)view firstRemoteVideoDecodedOfUid:(NSUInteger)uid size:(CGSize)size
{
    for (UIView *vc  in self.videoSessions) {
        if (vc) {
...
//用户离线，退出，退出或成为受众的原因
//- (void)didOfflineOfUid:(NSUInteger)uid reason:(TalkfunUserOfflineReason)reaso
//{
//    
//    for (UIView *vc in self.videoSessions) {
//        if (vc.tag ==uid) {
//            
//            [vc removeFromSuperview];
//            [self.videoSessions removeObject:vc];
//            
//            return;
//        }
//    }
...
- (TalkfunScoreView*)ScoreView
{
    if(_ScoreView==nil){
        ...
        _ScoreView.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height);
