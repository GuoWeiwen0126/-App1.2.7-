...
- (void)buttonClick:(ChoiceButton *)button
{
    ...
    //    NSArray *NetworkSelectionArray = self.networkSelectionArray[@"data"];
    //
    //    NSDictionary *dict =  NetworkSelectionArray[Index];
    //
...
- (void)networkSpeed:(id)obj{
    ...
    self.networkSpeedLabel.text = [NSString stringWithFormat:@"当前网络%@",obj[@"speed"]];
    if (networkStatus == TalkfunNetworkStatusGood) {
        self.networkSpeedLabel.textColor = [UIColor greenColor];
