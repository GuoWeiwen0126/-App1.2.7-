...
//        [self.answerHeightDict setObject:arr forKey:@(self.dataSource.count - 1)];
    ...
    {
        NSString * xid = [NSString stringWithFormat:@"%@",params[@"mess"][@"xid"]];
        
        if (![self.xid isEqualToString:xid] && (![params[@"mess"][@"role"] isEqualToString:TalkfunMemberRoleAdmin] && ![params[@"mess"][@"role"] isEqualToString:TalkfunMemberRoleSpadmin])) {
...
{
    TFQuestionModel * model = [TFQuestionModel mj_objectWithKeyValues:self.dataSource[indexPath.row]];
