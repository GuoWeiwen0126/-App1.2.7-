...
@interface TalkfunLEEStarRating ()
...
@implementation TalkfunLEEStarRating
{
    CGSize starSize;
...
- (void)setFrame:(CGRect)frame{
    ...
    if (frame.size.height != starSize.height) frame = CGRectMake(frame.origin.x,( self.height - starSize.height)/2, frame.size.width, starSize.height);
...
- (void)setCurrentScoreChangeBlock:(void (^)(CGFloat currentScoreChange ,NSInteger tag ))currentScoreChangeBlock{
    
    _currentScoreChangeBlock = currentScoreChangeBlock;
    
    if (currentScoreChangeBlock) currentScoreChangeBlock(self.currentScore ,self.tag);
}
...
//    (self.maximumScore/starCount)/self.maximumScore
    if (ratio < 0) return;
...
//    if(currentScore<self.maximumScore/starCount){
//        currentScore = self.maximumScore/starCount;
//    }
//
    ...
    if (_currentScore != currentScore) {
        ...
        if (self.currentScoreChangeBlock)
            ...
            self.currentScoreChangeBlock(self.currentScore,self.tag);
    }
