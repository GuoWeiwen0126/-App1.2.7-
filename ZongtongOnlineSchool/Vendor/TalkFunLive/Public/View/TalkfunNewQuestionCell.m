...
- (void)configCell:(NSDictionary *)dict isPlayback:(BOOL)isPlayback{
    TFQuestionModel * model = [TFQuestionModel mj_objectWithKeyValues:dict];
    NSString * nameTime = [TalkfunUtils getUserNameAndTimeWith:dict playback:isPlayback];
    ...
    self.nameTimeLabel.text = [NSString stringWithFormat:@"%ld)%@",self.number,nameTime];
