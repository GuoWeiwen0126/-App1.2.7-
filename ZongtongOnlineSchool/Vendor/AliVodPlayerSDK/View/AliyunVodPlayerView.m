...
- (instancetype)initWithFrame:(CGRect)frame {
    return [self initWithFrame:frame andSkin:AliyunVodPlayerViewSkinBlue isLive:self.isLive];
}
...
- (void)initView{
    ...
    //直播 or 录播
    [self.controlView setIslive:self.isLive];
//    [self.controlView setEnableGesture:self.isLive ? NO:YES];
    self.moreView.downLoadBtn.hidden = YES;
...
#pragma mark - 指定初始化方法
- (instancetype)initWithFrame:(CGRect)frame andSkin:(AliyunVodPlayerViewSkin)skin isLive:(BOOL)isLive {
...
//            NSLog(@"播放器测试：TouchUpInside 跳转到%.1f",progressValue*self.aliPlayer.duration);
            AliyunVodPlayerState state = [self playerViewState];
        ...
        case UIControlEventTouchUpOutside:{
            ...
            [self.aliPlayer seekToTime:progressValue*self.aliPlayer.duration];
//            NSLog(@"播放器测试：TouchUpOutside 跳转到%.1f",progressValue*self.aliPlayer.duration);
            AliyunVodPlayerState state = [self playerViewState];
            if (state == AliyunVodPlayerStatePause) {
                [self.aliPlayer resume];
            }
        ...
        case UIControlEventTouchDownRepeat:{
            ...
            [self.aliPlayer seekToTime:progressValue*self.aliPlayer.duration];
//            NSLog(@"播放器测试：DownRepeat跳转到%.1f",progressValue*self.aliPlayer.duration);
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                //在播放器回调的方法里，防止sdk异常不进行seekdone的回调，在3秒后增加处理，防止ui一直异常
                self.mProgressCanUpdate = YES;
            });
